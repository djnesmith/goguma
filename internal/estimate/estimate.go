...
// or a match pattern that stopped matching), and its job is to stop WakeGuard
...
func Compute(job *model.Job, runs []model.Run, cfg config.Config) Result {
	...
	if job != nil && !job.Detection.Observable() && !hasUsableHistory(runs, cfg) {
		hold := cfg.WakeOnlyHold.D()
		if job.MaxRuntime > 0 {
			hold = job.MaxRuntime.D()
		}
		return Result{
			Ceiling: hold,
			// Says what happens, not what the mode is called. "The job
			// cannot be observed" describes WakeGuard's predicament;
			// what the user needs is that the Mac stays up for a set
			// time whether the job takes that long or not.
			Reason: "WakeGuard can't tell when this job starts or finishes, " +
	...
	if job != nil && job.MaxRuntime > 0 {
		return Result{
			...
			Typical: medianOf(completedDurations(runs)),
			Samples: len(runs),
...
func Summarize(job *model.Job, runs []model.Run, cfg config.Config) model.Stats {
	...
	for i := range runs {
		...
		// than a duration standing in for one. Runs on AC contribute nothing
		// because they cost nothing.
		drained += r.Drained()
	}
	st.BatteryDrained = drained
