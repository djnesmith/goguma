// Command wakeguard-daemon is the unprivileged background service.
//
...
func main() {
	...
	if err := layout.EnsureDirs(); err != nil {
		fmt.Fprintf(os.Stderr, "wakeguard-daemon: cannot prepare %s: %v\n", layout.StateDir, err)
		os.Exit(1)
	}

	log, closeLog, err := newLogger(*logFile, *verbose)
	if err != nil {
		fmt.Fprintf(os.Stderr, "wakeguard-daemon: %v\n", err)
		os.Exit(1)
	}
	defer closeLog()
