// Command wakeguard-mark wraps a scheduled job so WakeGuard knows exactly
// when it starts and stops.
//
//	wakeguard-mark <job-name> -- <command> [args...]
//
...
const usage = `wakeguard-mark — tell WakeGuard exactly when a job runs

usage:
  wakeguard-mark <job-name> -- <command> [args...]

example, in a crontab line:
  0 9 * * * wakeguard-mark morning-briefing -- hermes cron run morning-briefing

The command runs unchanged and its exit status is passed through. If the
WakeGuard background service is not running, the command still runs normally.
`
...
func main() {
	...
	if len(rest) == 0 {
		fmt.Fprintf(os.Stderr, "wakeguard-mark: no command given after %q\n\n%s", jobName, usage)
		os.Exit(2)
	}
...
func run(jobName string, argv []string) int {
	...
	if err := cmd.Start(); err != nil {
		fmt.Fprintf(os.Stderr, "wakeguard-mark: cannot run %s: %v\n", argv[0], err)
	...
	// signals it would have received without the wrapper.
	...
	if !resp.Known {
		if name, ok := jobNameOf(payload); ok {
			fmt.Fprintf(os.Stderr,
				"wakeguard-mark: no job named %q is registered — this run is not being tracked.\n"+
					"  register it with: wakeguard add --name %s --cron '<schedule>' --detection mark\n",
				name, name)
		}
	}
}
