...
struct JobsWindowView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if store.connection.blocksContent {
                ...
                EmptyStateView(
                    ...
                    detail: "Add a job, or import the ones you already have with `wakeguard import`."
                )
            } else {
                searchField
                ThemeHairline()
        ...
        .confirmationDialog(
            ...
            if pendingRemoval?.job.managed == true {
                Text(
                    "WakeGuard adopted this job automatically, so the next sync will add it straight "
                ...
                Text(
                    "WakeGuard will stop waking the Mac for this job. Its run history is kept, and the "
    ...
    private func statRow(_ view: JobView) -> some View {
        HStack(alignment: .top, spacing: Theme.Space.lg) {
            ...
            stat(
                "Battery used",
                view.stats.batteryDrained > 0 ? "\(view.stats.batteryDrained)%" : "none",
                tint: view.stats.batteryDrained > 0 ? Theme.Colors.warning : nil,
                help: "Charge spent holding the Mac awake for this job, added up across "
                    + "its recent runs. Measured from the battery level at each end of a "
                    + "hold, so runs on AC power count for nothing — they cost nothing."
            )
            Spacer(minLength: 0)
        }
    }
