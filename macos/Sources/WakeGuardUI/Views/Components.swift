...
///
/// This is load-bearing: WakeGuard's daemon is a separate process that may
...
struct DaemonUnavailableView: View {
    ...
    var body: some View {
        VStack(spacing: Theme.Space.md) {
            ...
            Text(Format.noWidow(error?.errorDescription ?? "WakeGuard isn't running."))
            ...
            if let hint = error?.recoveryHint, !hint.isEmpty {
                ...
                Text("Run `wakeguard install` in Terminal to set it up.")
                    .font(Theme.Typography.caption)
                    .foregroundStyle(Theme.Colors.textSecondary)
                    .multilineTextAlignment(.center)
                    .textSelection(.enabled)
        ...
        if !warnings.isEmpty {
            VStack(alignment: .leading, spacing: Theme.Space.xs) {
                ...
                if warnings.count > Self.maxShown {
                    Text("+\(warnings.count - Self.maxShown) more — run `wakeguard doctor`")
                        .font(Theme.Typography.caption)
                        .foregroundStyle(Theme.Colors.textSecondary)
...
/// Marks a job WakeGuard adopted for the user, rather than one they added.
