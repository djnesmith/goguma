...
/// The menu bar popover: what WakeGuard is doing right now, what it will do
...
struct PopoverView: View {
    ...
    private var headline: String {
        switch store.state {
        case .disconnected: "wakeguard isn't running"
        case .paused: "paused"
    ...
    private var subheadline: String? {
        switch store.state {
        case .disconnected:
            return "Run `wakeguard install` in Terminal."
        case .paused:
    ...
    /// Four, not six.
    ///
    /// `MenuBarExtra` keeps its bottom edge fixed when it resizes, so every
    /// point the content grows pushes the top edge *upward* — past the menu bar
    /// and out of sight if it grows far enough. The job list is the only part
    /// that grows, so its length is the lever that decides how far.
    private static let jobsShownInPopover = 5
    ...
    private var jobsList: some View {
        VStack(alignment: .leading, spacing: Theme.Space.xs) {
            if store.jobs.isEmpty {
                Text("No jobs registered. Add one from the Jobs window, or import your crontab with `wakeguard import`.")
                    .font(Theme.Typography.caption)
                    .foregroundStyle(Theme.Colors.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
