...
/// WakeGuard exists to catch, and those rows are called out explicitly.
struct HistoryWindowView: View {
        ...
        if store.connection.blocksContent || (loadError?.isUnreachable ?? false) {
            ...
            VStack(alignment: .leading, spacing: Theme.Space.md) {
                ...
                RunDurationChart(
                    ...
                    wakeBuffer: store.config?.wakeBuffer ?? .zero
                )
            ...
            EmptyStateView(
                ...
                detail: isLoading ? nil : "This job hasn't run since WakeGuard started watching it."
            )
        }
    }
    ...
    private func summary(_ stats: Stats) -> some View {
        HStack(alignment: .top, spacing: Theme.Space.xl) {
            ...
            statBlock(
                "Battery used",
                stats.batteryDrained > 0 ? "\(stats.batteryDrained)%" : "none",
                tint: stats.batteryDrained > 0 ? Theme.Colors.warning : Theme.Colors.textPrimary,
                note: "charge spent holding the Mac awake for this job"
            )
            Spacer(minLength: 0)
        }
        .padding(.horizontal, Theme.Space.md)
    ...
    private func costTint(_ stats: Stats) -> Color {
        stats.batteryDrained > 0 ? Theme.Colors.warning : Theme.Colors.textPrimary
    }
        ...
        Table(runs) {
            ...
            TableColumn("Woke Mac") { run in
                Image(systemName: run.wokeMachine ? Theme.Icon.ok : Theme.Icon.outcomeUnknown)
                    ...
                    .help(
                        run.wokeMachine
                            ? "The Mac was asleep and WakeGuard woke it, so this run would have been missed."
