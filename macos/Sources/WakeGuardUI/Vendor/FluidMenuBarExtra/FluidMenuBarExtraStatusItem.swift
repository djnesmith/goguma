...
final class FluidMenuBarExtraStatusItem: NSObject, NSWindowDelegate {
    ...
    /// WakeGuard's only change to the vendored file.
    ///
    /// The upstream package sets the button's image once, at init. WakeGuard's
    /// menu bar glyph changes with what the daemon reports — the bear sleeps or
    /// wakes, and it turns red on a thermal cutout — so the button has to stay
