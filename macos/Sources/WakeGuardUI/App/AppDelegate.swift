...
/// main menu, and gets out of the way. WakeGuard's app does no privileged work
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func installMenuBarItem() {
        let window = FluidMenuBarExtraWindow(title: "WakeGuard") {
    ...
    func applicationWillTerminate(_: Notification) {
        // Leave the daemon alone. Unlike some menu bar apps, quitting WakeGuard
...
enum MainMenu {
    static func build() -> NSMenu {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(
            withTitle: "About WakeGuard",
            action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)),
            keyEquivalent: ""
        )
        appMenu.addItem(.separator())
        appMenu.addItem(
            withTitle: "Hide WakeGuard",
            action: #selector(NSApplication.hide(_:)),
            keyEquivalent: "h"
        )
        appMenu.addItem(.separator())
        appMenu.addItem(
            withTitle: "Quit WakeGuard",
