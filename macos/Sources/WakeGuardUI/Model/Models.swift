...
enum DetectionMode: String, LenientRawEnum {
    /// Exact: the job is wrapped in `wakeguard-mark`, which pings the daemon on
    ...
    var explanation: String {
        switch self {
        case .mark:
            "Exact. Wrap the job in `wakeguard-mark` in your crontab; it reports the real start, "
                + "exit, and exit code, so the ceiling is accurate from the first run."
        case .pattern:
            "Best effort. WakeGuard scans the process table for a regular expression. No change "
                + "to your crontab, but a wrong pattern means the job is never seen."
        case .wakeOnly:
            "Not watched. WakeGuard wakes the Mac and holds sleep off for a fixed window, then "
                + "releases. Use this when the job runs inside another app's own process, so there "
                + "is nothing to wrap and no process to match. The window can't self-tune, which is "
                + "the honest price of not being able to see the job."
        case .unknown:
...
enum WarningKind: String, LenientRawEnum {
    ...
    var isCritical: Bool {
        switch self {
        case .neverDetected, .wakeFailed, .scheduleParse: true
...
/// A registered schedule that WakeGuard holds a wake window for. WakeGuard
...
struct Job: Codable, Sendable, Hashable, Identifiable {
    ...
    /// True when WakeGuard adopted this job automatically from a scheduler it
...
struct Stats: Codable, Sendable, Hashable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case batteryDrained = "battery_drained"
    ...
    init(from decoder: any Decoder) throws {
        ...
        batteryDrained = c.value(.batteryDrained, 0)
    ...
    func encode(to encoder: any Encoder) throws {
        ...
        try c.encode(batteryDrained, forKey: .batteryDrained)
...
struct DaemonStatus: Codable, Sendable, Hashable {
    ...
    /// Non-empty when WakeGuard is deliberately not registering a wake — a
    ...
    var actionableWarnings: [DaemonWarning] {
        ...
        // The daemon reports a failed wake through `wake_error`, outside the
        // warnings array. A wake that was never registered is the single most
        // important failure in the product (jobs are silently missed), so it is
        // promoted to the top.
        //
        // But only when it is actually a failure. A non-empty `wake_suppressed`
        // is the daemon saying "I chose not to schedule one", and dressing that
        // up as an error would train users to ignore this list.
        if wakeSuppressed.isEmpty,
...
struct JobView: Codable, Sendable, Hashable, Identifiable {
    ...
    /// but `ok` there means "WakeGuard held a window and closed it cleanly",
    /// not "the job succeeded". WakeGuard never sees a wake-only job at all, so
...
/// Reply to `sync`. A negative count means jobs were *retired* — their source
/// entry vanished — rather than added, which is why this is one signed number
/// and not two fields.
struct SyncResponse: Decodable, Sendable {
    var added: Int

    enum CodingKeys: String, CodingKey { case added }
    ...
    var summary: String {
        if added > 0 {
            return "Adopted \(Format.count(added, "new job"))."
        }
        if added < 0 {
...
/// Whether a job worked the last time it ran, as far as WakeGuard can tell.
enum RunStatus: Sendable, Hashable {
    ...
    var label: String {
        ...
        case .notWatched: "Not watched — WakeGuard can't tell"
        }
    }
}
