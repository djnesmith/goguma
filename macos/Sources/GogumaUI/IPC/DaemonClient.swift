...
/// Typed access to the goguma daemon.
///
...
final class DaemonClient: Sendable {
    ...
    private let queue = DispatchQueue(label: "glass.goguma.ui.ipc", qos: .utility)
...
extension DaemonClient {
    ...
    /// daemon's own `auto_adopt_interval`. The reply carries separate
    /// adopted/updated/retired counts (older daemons sent one signed
    /// `added`, which SyncResponse still tolerates).
