...
final class FluidMenuBarExtraWindow<Content: View>: NSPanel {
    ...
    private func contentSizeDidUpdate(to size: CGSize) {
        ...
        DispatchQueue.main.async { [weak self] in
            // `animate: false`, goguma's change.
            //
            // SwiftUI is already animating the content height, so this fires
            // once per frame with an intermediate size. Animating *toward*
            // each of those starts a fresh window animation every frame, so
            // the frame lags behind the content it is chasing and the window's
            // own background shows through the gap, a grey band that appeared
            // while the job list collapsed. Following the content exactly, one
            // frame at a time, is what makes the resize read as one motion.
